...
function generateTagCreator(tag: Tag, imports: Import[]): string {
    ...
    const newArgs: Type[] = List.filter(
        filterTypes,
        List.map(function (arg: any) {
            return arg.type;
        }, tag.args)
...
function generateTopLevelType(type_: Type, imports: Import[]): string {
    switch (type_.kind) {
        ...
        case "FixedType": {
            ...
            if (name === "List") {
                ...
                if (
                    ...
                    const generatedArgs: string[] = List.map(function (
                        arg: any
                    ) {
                        return generateTopLevelType(arg, imports);
                    }, args);
                    ...
                    const genericArgs: Type[] = List.foldl(
                        function (arg: any, xs: any) {
                            return List.append(xs, getGenericArgs(arg));
                        },
                        [],
                        args
                    );
                    const generatedGenericArgs: string[] = List.map(function (
                        arg: any
                    ) {
                        return generateType(arg, imports);
                    }, genericArgs);
        ...
        case "FunctionType": {
            const { args } = type_;
            const typeToReturn: string = generateType(
                args[args.length - 1],
                imports
            );
            const parts: string[] = List.indexedMap(
                function (arg: any, index: any) {
...
function getGenericTypes(type_: Type): GenericType[] {
    switch (type_.kind) {
        ...
        case "FixedType": {
            const { args } = type_;
            return List.foldl(
                function (newType: any, collection: any) {
    ...
    switch (type_.kind) {
        ...
        case "FixedType": {
            ...
            if (name === "List") {
                ...
                const genericArgs: Type[] = removeDuplicateTypes(
                    List.foldl(
                        function (arg: any, xs: any) {
                ...
                const qualifiedName: string = typeHasOverlapWithImportedModule(
                    type_,
                    imports
                )
                    ? `${type_.name}.${type_.name}`
...
function generateElseIfStatement(
    ...
    const maybeLetBody: string = generateLetBlock(
        elseIfStatement.letBody,
        parentTypeArguments,
        []
    );
...
function generateIfStatement(
    ...
    const ifBodySuffix: string = (function (): any {
        switch (ifStatement.ifBody.kind) {
            case "IfStatement": {
                return "";
            }
            default: {
    ...
    const elseBodySuffix: string = (function (): any {
        switch (ifStatement.elseBody.kind) {
            case "IfStatement": {
                return "";
            }
            default: {
    ...
    const maybeIfLetBody: string = generateLetBlock(
        ifStatement.ifLetBody,
        parentTypeArguments,
        []
    );
    const maybeElseLetBody: string = generateLetBlock(
        ifStatement.elseLetBody,
        parentTypeArguments,
        []
    );
    const predicate: string = generateExpression(ifStatement.predicate);
    const ifBody: string = generateExpression(
        ifStatement.ifBody,
        parentTypeArguments
    );
    ...
    const elseBody: string = generateExpression(
...
function generateExpression(
    ...
    switch (expression.kind) {
        ...
        case "IfStatement": {
            const actualParentTypeArguments: string[] =
                parentTypeArguments || [];
...
function generateFunction(
    ...
    const flattenedArgTypeArguments: string[] = List.foldl(
        function (xs: any, ys: any) {
            return List.append(ys, xs);
        },
        [],
    ...
    const typeArguments: string[] = List.append(
        flattenedArgTypeArguments,
        collectTypeArguments(function_.returnType)
    );
    const filteredTypeArguments: string[] = typeArguments.filter(function (
        value: any,
        index: any,
        arr: any
    ) {
    ...
    const maybeLetBody: string = generateLetBlock(
        function_.letBody,
        List.append(filteredTypeArguments, parentTypeArguments),
        imports
    );
    ...
    const maybeDoBody: string =
        function_.doBody !== null
            ...
            : "";
    const returnType: string = isAsync
    ...
    const bodyPrefix: string = isSimpleBody ? "return " : "";
    const bodySuffix: string = isSimpleBody ? ";" : "";
    const joinedTypeArguments: string[] = List.append(
        typeArguments,
        parentTypeArguments
    );
    const allParentTypes: Type[] = List.append(parentTypes, [
    ...
    const typeArgumentsString: string =
        filteredTypeArguments.length === 0
            ? ""
