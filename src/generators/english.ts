...
function generateListValue(list: ListValue): string {
    if (list.items.length === 0) return `An empty list`;
    ...
    switch (branchPattern.kind) {
        ...
        case "ListDestructure": {
            return branchPattern.parts
                ...
                .join(" is in of a list element from ");
        }
        case "Default": {
...
function generateFunction(function_: Function): string {
    const functionArguments = function_.args
        .map((arg) => {
            switch (arg.kind) {
                case "FunctionArg":
                    return `${arg.name} with the type ${generateType(
                        arg.type
                    )}`;
                case "AnonFunctionArg":
                    return `_${arg.index} with the type ${generateType(
                        arg.type
                    )}`;
            }
        })
...
${prefixLines(functionArguments, 4)}
...
export function generateEnglish(module: Module): string {
    return module.body
        .map(generateBlock)
