...
function blockKindStep(
    block: string,
    validators: Validator[]
): Result<string, BlockKinds> {
...
function stepMultilineComment(
    ...
    if (line === "-}") {
        ...
        const block: UnparsedBlock = createUnparsedBlock(
            "MultilineComment",
            infoWithCurrentLine.lineStart,
            infoWithCurrentLine.currentBlock
        );
...
function indentOrDefinitionStep(
    lineNumber: number,
    info: IntoBlockInfo,
    line: string,
    lines: string[]
    ...
    const nextLines: string[] =
        lineNumber > 0 && info.previousLine.trim() === ""
            ...
            if (lines.length >= 1) {
                ...
                if (line.trim().length === 0) {
                    ...
                    switch (info.currentBlock.length) {
                        ...
                        default: {
                            const isInMultilineComment: boolean =
                                info.currentBlockKind.kind === "Ok" &&
                                info.currentBlockKind.value ===
                                    "MultilineComment";
                            if (isInMultilineComment) {
                                return stepMultilineComment(
                                    lineNumber,
                                    info,
                                    line,
                                    xs
                                ...
                                const currentLineBlockKind: Result<
                                    string,
                                    BlockKinds
                                > = blockKind(line);
                                const isIndent: boolean =
                                ...
                                if (isIndent || isDefinition) {
                                    ...
                                    switch (info.currentBlockKind.kind) {
                                        case "Ok": {
                                            const { value } =
                                                info.currentBlockKind;
                                            const hasSpeech: boolean =
                                                (function (y: any) {
                                                    return y.length > 0;
                                                })(
                                                    info.currentBlock.filter(
                                                        function (line: any) {
                                                            return (
                                                                line.indexOf(
                                                                    `"`
...
function typeBlocks(blocks: Block[]): TypedBlock[] {
    return List.filter(function (block: any) {
        return (
            block.kind === "UnionType" ||
            block.kind === "TypeAlias" ||
            block.kind === "UnionUntaggedType"
        );
    }, blocks);
}
...
function exportTests(module: Module): MyExport {
    ...
    const namesToExpose: string[] = isTest
        ? List.filter(
              function (name: any) {
              ...
              List.map(
                  ...
                  List.filter(function (block: any) {
                      return (
                          block.kind === "Function" || block.kind === "Const"
                      );
                  }, module.body)
              )
          )
    ...
    const exportNames: string[] = List.foldl(
        function (export_: any, allNames: any) {
